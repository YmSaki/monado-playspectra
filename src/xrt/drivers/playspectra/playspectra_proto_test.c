...
main(void)
{
	...
	CHECK(approx(s.head.position[0], 1) && approx(s.head.position[1], 2) && approx(s.head.position[2], 3),
	      "1 position");
	CHECK(approx(s.head.orientation[3], 1), "1 orientation w=1");
	CHECK(s.head.position_valid && s.head.orientation_tracked, "1 flags true");
	...
	CHECK(s.head.present && s.head.position_tracked == false, "2 position_tracked=false");
	CHECK(s.head.orientation_valid == true, "2 missing flag defaults true");
